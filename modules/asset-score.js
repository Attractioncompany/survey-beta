...
export const TYPES = {
  ...
  우아한:   { T:+0.5, D:-0.1, M:+0.15 },   // D −0.5→−0.1: 8-27 COORD 개정의 전파 누락분 (감사 2026-08-29 발견)
  에너제틱: { T:-0.6, D:-0.4, M:-0.90 },
  화려함:   { T:-0.8, D:+0.8, M:+0.65 },
  클래식:   { T:-0.5, D:+0.1, M:+0.40 },   // D +0.4→+0.1: 동상. czm-core COORD가 정본 — 두 사본이 갈리면 20.3%의 타입이 파일 선택에 좌우된다
...
export function selfCheck(){
  ...
  /* 임계 1.5→1.2 (2026-08-29 좌표 단일화 때 갱신). 1.5는 옛 좌표(우아 D−0.5) 기준
     스냅샷이었다 — 8-27 정본(D−0.1)에서는 D축도 여름쌍을 일부 가르므로 2축 거리가
     0.76으로 늘어 비율이 1.45가 된다. 검사의 본질은 "M축이 여름쌍 거리를 실질적으로
     늘리는가"이고, 1.45배는 여전히 그렇다 — M축 신설 근거는 유지된다. */
  ok(with3>with2*1.2, `여름쌍이 M축으로 벌어져야 함 (2축 ${with2.toFixed(2)} → 3축 ${with3.toFixed(2)})`);
