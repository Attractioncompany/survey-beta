      ...
      if (ax >= 0) {
        ...
        return [pair
          ? "지금 좌표가 가운데로 모였습니다.\n" + B(pair[0]) + (jong(NAMES[pair[0]]) ? "과 " : "와 ")
            + B(pair[1]) + (jong(NAMES[pair[1]]) ? "이" : "가") + " 나란히 크게 자리 잡은 결과예요."
          : "지금 좌표가 가운데로 모였습니다.\n서로 반대쪽 매력이 나란히 자리 잡은 결과예요.",
          "한쪽을 조금 더 세우면 인상이 또렷하게 잡힙니다."];
      }
    }
    // ② 집중형
    if (spread >= 6)
  ...
  var WIDTH = {
    interocular: {
      hi: ["눈 사이가 넉넉해서 얼굴 가운데가 시원하게 트입니다.",
           "눈 사이가 넉넉해서 얼굴 가운데에 여백이 생겨요. 인상이 느긋하게 읽히는 배치죠."],
      lo: ["미간이 가까워서 얼굴 중심부가 짧아 보이고, 그만큼 눈에 시선이 먼저 갑니다.",
           "미간이 가까워 얼굴 중심이 촘촘하게 모여요. 눈썹 앞머리를 진하게 채우지 않으면 가운데가 한결 트여 보이죠."]
    },
    mouth_w: {
      hi: ["입이 넓게 자리 잡아 웃을 때 표정이 크게 퍼집니다.",
           "입이 넓게 자리 잡아 얼굴 아래쪽이 시원하게 열리고, 웃는 표정이 크게 퍼져요."],
      lo: ["입이 단정하게 모여 얼굴 아래쪽이 정돈돼 보입니다. 그만큼 시선은 눈매 쪽으로 올라가죠.",
           "입이 단정하게 모여 있어요. 아래쪽이 정돈되면서 시선이 눈매 쪽으로 올라가죠."]
    },
    face_HW: {
      hi: ["얼굴에 길이감이 있습니다. 목·어깨를 덮기보다 열어 두면 그 세로선이 그대로 이어지죠.",
    ...
    chin_len: {
      ...
      lo: ["턱이 짧게 자리 잡아 얼굴 아래쪽이 야무지게 모입니다. 이목구비가 가운데로 당겨 보이는 배치죠.",
           "턱이 짧게 자리 잡아 얼굴 아래쪽이 야무지게 모여요. 그만큼 이목구비가 가운데로 당겨 보이죠."]
    },
    parts_vpos: {
      ...
      hi: ["이목구비가 아래쪽에 모여 얼굴 무게가 아래로 내려앉고, 인상이 차분하게 읽힙니다.",
           "이목구비가 아래쪽에 자리를 잡았어요. 눈매에 힘을 주면 위아래 무게가 고르게 맞죠."],
      lo: ["이목구비가 위쪽에 자리해 얼굴 중심이 위로 당겨집니다. 정면에서 또랑또랑한 인상이 먼저 잡히죠.",
