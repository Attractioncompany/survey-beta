...
(function (root) {
  ...
  function rawRows(d) {
    ...
    // 코 행은 「코」 장으로 (2026-09-03 코·입 재분리 — 병합 시절 p="입" 잔재 정정, 대표 #4)
    one("코", "코 길이", q.nose_len, "코뿌리에서 코밑까지 (얼굴높이 대비)");
    one("코", "코 너비", r.nose_w, "콧방울 폭 (얼굴폭 대비)");
    one("코", "콧볼 두께", q.ala_thick, "콧볼 바깥 폭 대비 안쪽 폭");
    one("코", "콧구멍 노출", q.nostril_show, "코끝이 콧볼 아래선보다 위에 있는 정도 (얼굴높이 대비)");
  ...
  var WIDTH = {
    ...
    parts_vpos: {
      ...
      /* 인상어를 걷었다(이론 2026-09-03 · 인상어사전 §2-5). "차분하게"·"또랑또랑한"은
         parts_vpos에 등재 근거가 없고, 문헌 방향(동안 구성=이목구비 낮은 배치, Zebrowitz)과
         현행 관행 화법이 반대로 가는 소지까지 있다 — 이 필드는 배치 상태만 말한다. */
      hi: ["이목구비가 아래쪽에 모여 얼굴 무게가 아래로 내려앉는 배치입니다.",
           "이목구비가 아래쪽에 자리를 잡았습니다. 눈매에 힘을 주면 위아래 무게가 고르게 맞는 배치입니다."],
      /* lo에서 "아래쪽 여백" 류 서술은 쓰지 않는다 — 같은 장의 3분할 행과 기준이 달라
         이마 서술을 걷었던 것과 같은 충돌(하안부 짧음 ↔ 아래 여백)이 난다. 위치만 말한다. */
      lo: ["이목구비가 위쪽에 자리해 얼굴 중심이 위로 당겨진 배치입니다.",
           "이목구비가 위쪽에 자리를 잡았습니다. 얼굴 중심이 위로 당겨져 읽히는 배치입니다."],
  ...
  var MUTE_LINE = {
    ...
    mute_tentative:  ["채도가 낮고 회색이 살짝 섞인 톤다운 계열로 기울어 보이며, 드레이핑 확인은 아직입니다.",
                      "채도가 낮고 회색이 살짝 섞인 톤다운 계열로 기울어 보입니다. 아직 드레이핑 확인 전입니다."]
  };
